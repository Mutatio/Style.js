...
var Colors = {
	...
	getList: function () {
		var list = Colors.webSafe.extend(Colors);
	...
	getNamedList: function (websafeOnly) {
		var list = !websafeOnly ? this.getList() : this.webSafe;
		var out = {};

		for (var hex in list) {
			if (isString(list[hex])) {
				out[this.list[hex]] = hex;
			}
		}
...
Colors.list = Colors.getList();
Colors.listNamed = Colors.getNamedList();
Colors.listNamedWebSafe = Colors.getNamedList(true);
...
var Color = {
	...
	toType: function (value, type) {
		switch (type) {
			...
			case Color.XYZ:
			case Color.CIELAB:
				return global['to' + type](value);
		}
	},

	equals: function (color1, color2) {
		color1 = toHex(color1);
		color2 = toHex(color2);
...
Hex.isValid = function (hex) {
	if (hex) {
		...
		return hex.isHex() && hex.length.equals(3, 6);
	}
		...
		if (color !== undefined && color.isSet()) {
			...
			if (count > 1) {
				...
				for (var i = 2; i < count; ++i) {
					color.hue = Hue.shift(color.hue, i * angle);
...
function complement(color) {
	...
	if (type) {
		...
		if (color !== undefined && color.isSet()) {
			color.hue = Hue.shift(color.hue, 180);
...
function square(color) {
	...
	if (type) {
		...
		if (color !== undefined && color.isSet()) {
			...
			color.hue = Hue.shift(color.hue, 90);
...
function tetradic(color) {
	...
	if (type) {
		...
		if (color !== undefined && color.isSet()) {
			...
			color.hue = Hue.shift(color.hue, 60);

			out.push(Color.toType(color, type));

			color.hue = Hue.shift(color.hue, 120);
